...
interface RCommonLevels {
    levels: TLevel[];
}

interface RCommonLevelCategories {
...
const LevelsLayout = ({
    ...
    levels: TLevel[];
    setSelectedLevelId: React.Dispatch<React.SetStateAction<number | null>>;
}) => (
    <main className="overflow-x-hidden">
        <div className="bg-dark-2 p-[30px_18px_42px] flex items-center">
            <div>
                <h1 className="text-[22px] font-medium">Привет Зазаза!</h1>
                <p className="text-sm mt-3">
                    Этот квиз поможет вам запомнить слова легко
                </p>
            </div>

            <img
                className="m-[-41px_-39px_-58px_-66px]"
                ...
                {levels.map((level) => (
                    ...
                    >
                        ...
                        <div className="flex flex-col">
                            <span className="text-lg">{level.level_name}</span>
...
const CategoriesLayout = ({
    ...
    useEffect(() => {
        http.get<RCommonLevelCategories>(
            `/api/common/levels/${level_id}/categories`
        ).then(({ data }) => {
...
const GameLayout = ({
    ...
    useEffect(() => {
        http.post<RQuizzesNew>(
            `/api/quizzes/new?category_id=${category_id}&level_id=${level_id}`
        ).then(({ data: { questions } }) => {
...
const Home = () => {
    ...
    useEffect(() => {
        http.get<RCommonLevels>("/api/common/levels").then(({ data }) => {
